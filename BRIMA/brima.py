...
def execute_clicks():
    ...
    if coordenadas["CPF"]:
        ...
        time.sleep(1)  # Tempo para o duplo clique
        pyautogui.hotkey('ctrl', 'c')
        time.sleep(1)  # Tempo para copiar o conteúdo
        cpf_list.append(pyperclip.paste())
        cpf = str(pyperclip.paste()) if isinstance(pyperclip.paste().replace(" ",""), int) else pyperclip.paste().replace(" ","")
        print(cpf)
        time.sleep(4)
    ...
    if coordenadas["AGÊNCIA"]:
        ...
        agencia = str(pyperclip.paste()) if isinstance(pyperclip.paste().replace(" ",""), int) else pyperclip.paste().replace(" ","")
        print(agencia)
    ...
    if coordenadas["CONTA"]:
        ...
        conta = str(pyperclip.paste()) if isinstance(pyperclip.paste().replace(" ",""), int) else pyperclip.paste().replace(" ","")
        print(conta)
